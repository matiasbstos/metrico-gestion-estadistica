            ...
            filters: [
              {
                fieldFilter: {
                  ...
                  value: { integerValue: String(startMs) }
                }
              },
              {
                fieldFilter: {
                  ...
                  value: { integerValue: String(endMs) }
                }
              }
            ]
          }
...
async function run() {
  console.log("Descargando 2026 por quincenas...");
  const quincenas = [];
  for (let m = 0; m < 9; m++) {
    const y = 2026;
    quincenas.push({
  ...
  for (const q of quincenas) {
    const docs = await fetchRange(q.start, q.end);
    console.log(`${q.label}: ${docs.length} registros`);
    docs.forEach(doc => {
      const f = doc.fields || {};
      ...
      if (tAdm > 0) {
        allPacs.push({ id, corr, tAdm, isAlta });
      }
    });
  }
  ...
  // Deduplicar exactamente como MÉTRICO
  const pacMap = new Map();
  allPacs.forEach(p => {
    const correlativo = String(p.corr || p.id || '').replace(/\.0$/, '').trim();
    const key = correlativo ? `${correlativo}_${Math.floor(p.tAdm / 86400000)}` : p.id;
    if (!pacMap.has(key)) pacMap.set(key, p);
  });
  console.log(`Total pacientes 2026 deduplicados: ${pacMap.size}`);

  // Agrupar por turnos exactos
  ...
  console.log("\n==========================================================");
  console.log("=== VERIFICACIÓN EXACTA DEL TURNO 30/04/2026 (JUEVES) ===");
  console.log("==========================================================");
  allShifts.filter(s => s.fechaIso === '2026-04-30').forEach(s => {
    console.log(`Fecha: ${s.fechaTurno} (${s.fechaIso}) [${s.horario}] -> ${s.count} pac. (${s.altas} altas) [${s.tipo}]`);
  });
}
