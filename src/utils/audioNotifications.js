...
/**
 * Chime Armónico de Inicio de Sesión
 * Tonalidad Mayor Ascendente: C5 (523.25 Hz) -> E5 (659.25 Hz) -> G5 (783.99 Hz) -> C6 (1046.50 Hz)
 */
export const playLoginChime = () => {
  try {
    const ctx = getAudioContext();
    if (!ctx) return;
...
/**
 * Chime Seguro de Cierre de Sesión
 * Tonalidad Descendente: D5 (587.33 Hz) -> A4 (440.00 Hz) -> E4 (329.63 Hz)
 */
export const playLogoutChime = () => {
  try {
    const ctx = getAudioContext();
    if (!ctx) return;
